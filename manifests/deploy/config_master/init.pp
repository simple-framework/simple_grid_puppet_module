class simple_grid::deploy::config_master::init(
  ...
  $lightweight_components.each |Integer $index, Hash $lightweight_component| {
    ...
    exec{"Waiting for deployment of ${index} to end":
      command => "bolt task run simple_grid::deploy_status node_fqdn=${node_fqdn} deploy_status_file=${deploy_status_file} execution_id=${index} retry_interval=${retry_interval} max_retries=${max_retries} --modulepath /etc/puppetlabs/code/environments/simple/site/:/etc/puppetlabs/code/environments/simple/modules/ --nodes localhost > /etc/simple_grid/.${index}.status", #${node_fqdn} > /etc/simple_grid/${index}.status",
      path    => '/usr/local/bin/:/usr/bin/:/bin/:/opt/puppetlabs/bin/',
      user    => root
    }
