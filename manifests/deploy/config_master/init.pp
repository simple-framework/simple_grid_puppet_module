class simple_grid::deploy::config_master::init(
  ...
  $lightweight_components.each |Integer $index, Hash $lightweight_component| {
    ...
    exec{"Waiting for deployment of ${index} to end":
      command => "bolt task run simple_grid::deploy_status execution_id=${index} --modulepath /etc/puppetlabs/code/environments/simple/site/:/etc/puppetlabs/code/environments/simple/modules/ --nodes ${node_fqdn} > /etc/simple_grid/${index}.status",
      path    => '/usr/local/bin/:/usr/bin/:/bin/:/opt/puppetlabs/bin/',
      user    => root
    }
  }
