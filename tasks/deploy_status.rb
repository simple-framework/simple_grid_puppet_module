...
class DeployStatus <TaskHelper
    def send_probe(node_fqdn, deploy_status_file, execution_id)
    ...
    def task(node_fqdn:nil, deploy_status_file:nil, execution_id:nil, retry_interval:nil, max_retries:nil, **kwargs)
        retry_interval = retry_interval.to_i #10
        max_retries = max_retries.to_i #6
        trial = 0
        result = send_probe(node_fqdn, deploy_status_file, execution_id)
